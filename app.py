...
# ─────────────────────────────────────────────
FUNDS_GSHEET_ID   = "1FSgvIG6VsJxB5QPY6fmwAwGc1TYLB0KXg-7ckkD_RJQ"
SERVICE_GSHEET_ID = "1ty_tqcyGqmVI4pQZetHHKd-cC0O2HCpD2dbpNpYlPtY"
...
def _gsheet_to_bytes(sheet_id: str) -> Tuple[bytes, str]:
    """הורד Google Sheet כ-XLSX. מחזיר (bytes, error_message)."""
    ...
    try:
        ...
        return b"", f"שגיאה בהורדת גיליון גוגל ({sheet_id[:12]}...): {e}"
...
def _load_service_scores(xlsx_bytes: bytes) -> Tuple[Dict[str, float], str]:
    ...
    if "provider" not in df.columns or "score" not in df.columns:
        if len(df.columns) >= 2:
            ...
            return {}, "מבנה גיליון שירות לא ידוע – נדרשות עמודות provider + score"
    out = {}
...
@st.cache_data(show_spinner=False, ttl=600)   # מרענן כל 10 דקות
...
if df_long.empty:
    st.error(
        "❌ לא הצלחתי לטעון נתונים מ-Google Sheets.\n\n"
...
with st.expander(
    ...
    for i, mgr_name in enumerate(all_managers):
        with mgr_cols[i % 3]:
            ...
            svc_score  = service_map.get(mgr_name, 50)
            if st.checkbox(
                f"{mgr_name}  ·  {fund_count} קרנות  ·  שירות {svc_score:.0f}",
                value=checked,
